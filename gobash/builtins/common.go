...
///* **************************************************************** */
//
///* This is a lot like report_error (), but it is for shell builtins instead of shell control structures, and it won't ever exit
//   the shell. */
//
...
//}
//
///* Return if LIST is NULL else barf and jump to top_level.  Used by some builtins that do not accept arguments. */
//void no_args(list)
//	 WORD_LIST *list;
//{
//	if (list) {
...
//}
//
///* Check that no options were given to the currently-executing builtin, and return 0 if there were options. */
//int no_options(list)
//	 WORD_LIST *list;
//{
...
//}
//
///* Function called when one of the builtin commands detects an invalid option. */
//void sh_invalidopt(s)
//	 char *s;
//{
//	builtin_error(_("%s: invalid option"), s);
//}
//
//void sh_invalidoptname(s)
//	 char *s;
//{
...
///* **************************************************************** */
//
///* Convert a WORD_LIST into a C-style argv.  Return the number of elements in the list in *IP, if IP is non-null.  A convenience
//   function for loadable builtins; also used by `test'. */
...
//}
//
///* Remember LIST in $0 ... $9, and REST_OF_ARGS.  If DESTRUCTIVE is non-zero, then discard whatever the existing arguments are,
//   else only discard the ones that are to be replaced. */
//void remember_args(list, destructive)
//	 WORD_LIST *list;
//	 int destructive;
//{
//	register int i;
...
//	}
//
//	/* If arguments remain, assign them to REST_OF_ARGS. Note that copy_word_list (NULL) returns NULL, and that dispose_words
//	   (NULL) does nothing. */
//	if (destructive || list) {
...
///* **************************************************************** */
//
///* Read a numeric arg for this_command_name, the name of the shell builtin that wants it.  LIST is the word list that the arg is
//   to come from. Accept only the numeric argument; report an error if other arguments follow.  If FATAL is 1, call
//   throw_to_top_level, which exits the shell; if it's 2, call jump_to_top_level (DISCARD), which aborts the current command; if
//   FATAL is 0, return an indication of an invalid number by setting *NUMOK == 0 and return -1. */
...
//}
//
///* Return the octal number parsed from STRING, or -1 to indicate that the string contained a bad number. */
...
///* **************************************************************** */
//
///* Return a consed string which is the current working directory. FOR_WHOM is the name of the caller for error printing.  */
...
//					(for_whom && *for_whom) ? for_whom : get_name_for_error(), _(bash_getcwd_errstr), strerror(errno));
//			return (char *)NULL;
//		}
//	}
//
...
//void set_working_directory(name)
//	 char *name;
//{
//	FREE(the_current_working_directory);
...
//
//#if defined (JOB_CONTROL)
//int get_job_by_name(name, flags)
//	 const char *name;
//	 int flags;
//{
...
//				/* This is for the kill builtin.  POSIX.2 says the signal names are displayed without the `SIG' prefix. */
...
//			}
//#if defined (JOB_CONTROL)
//			/* POSIX.2 says that `kill -l signum' prints the signal name without the `SIG' prefix. */
...
//}
//
///* Return the function implementing the builtin NAME, but only if it is a POSIX.2 special builtin. */
...
//}
//
///* Sort the table of shell builtins so that the binary search will work in find_shell_builtin. */

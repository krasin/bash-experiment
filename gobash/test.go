...
/* Define PATTERN_MATCHING to get the csh-like =~ and !~ pattern-matching binary operators. */
/* #define PATTERN_MATCHING */

/* Values for the flags argument to binary_test */
const TEST_PATMATCH = 0x01
const TEST_ARITHEXP = 0x02
const TEST_LOCALE = 0x04
...
///* The following few defines control the truth and false output of each stage. TRUE and FALSE are what we use to compute the
//   final output value. SHELL_BOOLEAN is the form which returns truth or falseness in shell terms. Default is TRUE = 1, FALSE = 0, 
//   SHELL_BOOLEAN = (!value). */
//#define TRUE 1
//#define FALSE 0
...
//
//static void test_syntax_error(format, arg)
//	 char *format, *arg;
//{
...
//}
//
///* Syntax error for when an integer argument was expected, but something else was found. */
//static void integer_expected_error(pch)
//	 char *pch;
//{
//	test_syntax_error(_("%s: integer expression expected"), pch);
//}
//
///* Increment our position in the argument list.  Check that we're not past the end of the argument list.  This check is supressed 
//   if the argument is FALSE.  Made a macro for efficiency. */
...
//	switch (op) {
//	case OT:
//		return (r1 < r2 || (r2 == 0 && st1.st_mtime < st2.st_mtime));
//	case NT:
//		return (r1 > r2 || (r1 == 0 && st1.st_mtime > st2.st_mtime));
//	case EF:
...
//	}
//
//	/* All of the unary operators take an argument, so we first call unary_advance (), which checks to make sure that there is an
//	   argument, and then advances pos right past it.  This means that pos - 1 is the location of the argument. */
//	unary_advance();
//	return (unary_test(op, argv[pos - 1]));
//}
//
//int unary_test(op, arg)
//	 char *op, *arg;
//{
//	intmax_t r;
...
//
//	case 'N':
//		return (sh_stat(arg, &stat_buf) == 0 && stat_buf.st_atime <= stat_buf.st_mtime);
//
...
//
///* This could be augmented to handle `-t' as equivalent to `-t 1', but POSIX requires that `-t' be given an argument. */
